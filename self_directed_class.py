...
class selfDirectedStrategy:
    def __init__(self, longevity_table, scenarios, inflation_matrix, w_0, age_0, female_ind):
    ...
    def pre_annuity_payout_calc(self, scen, wealth, age, payout_matrix, wealth_at_death, is_ruined, options_dict):
        ...
        #SWR Strategy
        if(options_dict['Strategy Type'][0] == 'swr'):
            if(age == self.age_0):
                desired_drawdown = self.w_0*options_dict['Strategy Specification']['swr']
            else:
                desired_drawdown = self.w_0*options_dict['Strategy Specification']['swr']*np.cumprod(self.inflation_matrix[scen, 0:age-self.age_0]+1)[-1]
        ...
        #SWR + Harmonic
        if(options_dict['Strategy Type'][0] == 'swr_harmonic'):
            if(age == self.age_0):
                desired_drawdown = self.w_0*options_dict['Strategy Specification']['swr']
            else:
                desired_drawdown = self.w_0*options_dict['Strategy Specification']['swr']*np.cumprod(self.inflation_matrix[scen, 0:age-self.age_0]+1)[-1]
    ...
    def calc_table(self, options_dict, swrs=[.020, .025, .030, .035, .040, 0.042, 0.043, 0.044, 0.045, 0.046]):
        ...
        out = pd.DataFrame(np.zeros((2*len(swrs)+6, 1)), columns=[name])
        inds = ['Median Wealth at Death'] + ['Median Wealth at Start of Annuity'] + ['Median Wealth at Death Cond Die Before Annuity'] + ['Prob Ruin'] + ['Prob Ruin Cond Live to 80'] + ['Prob Ruin Cond Live to 90']
